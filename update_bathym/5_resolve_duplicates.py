"""
Local QA tool: resolve possible duplicate surveys flagged by 4_compute_thresh_depth.py
(same location + date, republished by USACE's eHydro under a second job ID --
see the DUPLICATE-SURVEY DETECTION block there).

Run after 6_review_surveys.py, once both copies in a duplicate pair have been
...
gpkg directly, so this must run before 9_make_depth_polygons.py deletes it --
i.e. before the next CI pipeline run, same as 6_review_surveys.py). Molly picks
...
# Same depth bins/colors as 6_review_surveys.py / 9_make_depth_polygons.py / app.py's
# DEPTH_POLY_COLORS -- duplicated rather than imported since 6_review_surveys.py runs
# top-level Dash-app setup on import (its own comment explains why it duplicates
# these too instead of importing from 9_make_depth_polygons.py). Keep in sync by hand.
DEPTH_BINS = [
    (20,   None,  ">20 ft"),
    (17.5, 20,    "17.5-20 ft"),
    (15,   17.5,  "15-17.5 ft"),
...
def _ensure_resolved_column(df):
    ...
    at_risk/problem_lon/problem_lat in 6_review_surveys.py -- an all-blank column
...
def build_duplicate_groups(df):
    """Union-find over the (asymmetric) duplicate_of links -- 4_compute_thresh_depth.py
