"""
One-off scoped run for a new batch Molly requested (2026-09-11): 4 explicitly
named surveys plus every survey in combine_group CC, FF, JJ, and Q from
channel_width_survey_list_combined.csv. Runs each ID through the same
conversion logic as 2_read_in_surveys.py -> 3_process_surveys.py ->
4_compute_thresh_depth.py (datum detection, NAVD88 conversion, depth_ft
threshold calc).

Deliberately NOT the real pipeline stages -- this does not touch
SurveyPointLayers/, NAVD88Files/, survey_metadata.csv, lm_ids_done.csv, or
bathym_fixed.csv. Standalone output only ("survey point layers with the
thresh depths calculated in a new folder"), skipping 5_resolve_duplicates.py
and 6_review_surveys.py entirely -- so nothing here is live on the map.
Output: one gpkg per survey (SurveyPoint schema + Z_navd88 + depth_ft +
survey_type) in data/SurveysToDownload/. The folder was cleared of the prior
batch's output before this run.

Re-running is safe: always redownloads and overwrites the output file for
each ID below.
"""
import io
import zipfile
...
# 4 explicitly named surveys
EXPLICIT_IDS = [
    "LM_05_VGG_20240807_CS_4274_4226",
    "LM_08_MVF_20240722_CS_4958_4940",
    "LM_09_LPM_20240725_CS_5234_5184",
    "LM_25_MADX_20230907_CS_917_915_SORT_ACTUALDEPTHS",
]

# + everything in combine_group CC, FF, JJ, Q from the combined survey list
COMBINED_CSV = SCRIPT_DIR / "channel_width_survey_list_combined.csv"
_combined = pd.read_csv(COMBINED_CSV)
GROUPS = ["CC", "FF", "JJ", "Q"]
GROUP_IDS = _combined.loc[_combined["combine_group"].isin(GROUPS), "survey_id"].tolist()

SURVEY_IDS = EXPLICIT_IDS + GROUP_IDS
print(f"{len(EXPLICIT_IDS)} explicit + {len(GROUP_IDS)} from groups {GROUPS} = {len(SURVEY_IDS)} total surveys")
