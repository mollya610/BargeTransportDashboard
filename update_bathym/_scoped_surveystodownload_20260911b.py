"""
Follow-up one-off scoped run (2026-09-11): 2 more surveys Molly named,
LM_08_MVF_20220921_CS and LM_08_MVF_20221212_CS. Neither has a mile-range
suffix in eHydro's own surveyjobidpk (confirmed via the eHydro_Survey_Data
FeatureServer query) -- that's their real, full survey ID, not a truncation.
Adds to the same SurveysToDownload/ batch from the CC/FF/JJ/Q + 4-explicit
run earlier today; does not clear or touch those 35 existing files. Same
...
bathym_fixed.csv. Standalone output only, skipping 5_resolve_duplicates.py
...
survey_type) in data/SurveysToDownload/.
...
SURVEY_IDS = [
    "LM_08_MVF_20220921_CS",
    "LM_08_MVF_20221212_CS",
]
print(f"{len(SURVEY_IDS)} surveys")
